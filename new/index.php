...
$stmt = $pdo->query('SELECT p.*, GROUP_CONCAT(pa.title ORDER BY pa.id) AS antworten FROM umfragen p LEFT JOIN umfrage_antwort pa ON pa.poll_id = p.id GROUP BY p.id');
